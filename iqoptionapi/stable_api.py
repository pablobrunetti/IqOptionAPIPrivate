...
class IQ_Option:
        ...
        while True:
            ...
            while True:
                if time.time() - start > 60:
                    logging.error('**warning** get_all_init late 60 sec')
                    break
                try:
    ...
    def unsubscribe_strike_list(self, ACTIVE, expiration_period):
        if ACTIVE in self.api.instrument_quites_generated_data : del self.api.instrument_quites_generated_data
    ...
    # Atualizado por Rafael (tg: @rafamadeira01)
    def get_digital_current_profit(self, ACTIVE = '', duration = 1):        
        self.api.subscribe_digital_profit(ACTIVE, duration)
        time.sleep(1)
        profit = self.api.instrument_quites_generated_data2
        self.api.unsubscribe_digital_profit(ACTIVE, duration)
        time.sleep(1)
        return profit[ACTIVE] if ACTIVE in profit and ACTIVE.strip() != '' else False if ACTIVE.strip() != '' else profit

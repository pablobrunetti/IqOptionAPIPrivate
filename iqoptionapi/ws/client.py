...
class WebsocketClient(object):
    ...
    def on_message(self, message): # pylint: disable=unused-argument
        ...
        #------for forex&cfd&crypto..
            ...
            #print(list(OP_code.ACTIVES.values()))        
            #print(list(OP_code.ACTIVES.values()).index(message["msg"]["asset_id"]))
            try:
                Active_name=list(OP_code.ACTIVES.keys())[list(OP_code.ACTIVES.values()).index(message["msg"]["asset_id"])]
            except:
                pass   
            if "SPT" in str(message):
                candle = message["msg"]["prices"][-1]               
				
                askPrice=(float)(candle["call"]["ask"])
                ProfitPercent=((100-askPrice)*100)/askPrice
                try:
                    self.api.instrument_quites_generated_data2[Active_name]=ProfitPercent     
                except:
                    pass
        else:
            pass
